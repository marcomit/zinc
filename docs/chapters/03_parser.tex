...
Errors are managed through a separate bookkeeping structure. The parser maintains a stack of \epth{breakpoints}: when entering an alternative, a breakpoint is pushed; if parsing succeeds the breakpoint is committed, otherwise it is popped and the accumulated errors for that attempt are discarded.
